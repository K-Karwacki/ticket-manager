...
public class CoordinatorCardController {
    ...
    private void assign(ActionEvent event) {
        ...
        System.out.println(this.event.getAssignedCoordinators());
        if(this.event.getAssignedCoordinators().contains(user)){
        ...
        Pair<Parent, CoordinatorListPopupController> p = fxmlManager.getFXML(COORDINATOR_LIST_POPUP);
    ...
    public void setButton(){
        if(event.getAssignedCoordinators().contains(user)){
            setButtonToActive();
        }
    }
